...
type CloseEventArgType<T> = CloseEventArgTypeComplex<T> | CloseEventArgTypeSimple<T>
...
function _useOverlay() {
  ...
  const create = <T extends Component>(
    component: T, _options?: OverlayOptions<ComponentProps<T>>,
  ): OverlayInstance<T> => {
    const {
      defaultOpen,
      destroyOnClose,
    ...
    const options = reactive<Overlay>({
      ...
      isMounted: Boolean(defaultOpen),
      isOpen: Boolean(defaultOpen),
      destroyOnClose: Boolean(destroyOnClose),
      originalProps: props || {},
      props: {
        ...props,
      },
