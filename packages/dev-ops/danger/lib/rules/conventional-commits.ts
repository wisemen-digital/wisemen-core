...
export const conventionalCommitsRule = createRule(
  ...
  async ({ danger, config }) => {
    ...
    if (commits.length === 0) {
      return { passed: true, message: 'No commits to check' };
    }
    ...
    if (violations.length === 0) {
      return { passed: true, message: `All ${commits.length} commit(s) follow conventional commits` };
    }
    
    const message = violations.join('\n');
    return { passed: false, message, type: ResultType.WARN };
  },
  {
