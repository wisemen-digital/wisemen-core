...
import { FutureInfinityDate, PastInfinityDate, WiseDate } from '@wisemen/wise-date'
import { InvalidDateRangeBounds } from '#src/date-range-errors.js'
...
describe('DateRange unit tests', () => {
  ...
  describe('diff', () => {
    ...
    it('diff between (-infinity, +infinity) and [today, today] is (-infinity, yesterday] and [tomorrow, +infinity)', () => {
      ...
      expect(diff[1].isSame(new DateRange(WiseDate.tomorrow(), new FutureInfinityDate())))
        .toBe(true)
    })
  ...
  describe('startsAfter', () => {
    it('returns true if start date is after the provided date', () => {
      ...
      const yesterday = WiseDate.yesterday()
    ...
    it('returns false if start date is before the provided date', () => {
      ...
      const tomorrow = WiseDate.tomorrow()
