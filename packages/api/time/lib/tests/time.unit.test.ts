...
import { InvalidAbsoluteSeconds, InvalidTimeString } from '#src/time-error.js'
...
describe('Time class', () => {
  describe('isValidTimeString', () => {
    ...
    it('When sending an invalid time string, it should return false', () => {
      ...
      expect(Time.isValidTimeString('1:1')).toBe(false)
      expect(Time.isValidTimeString('25:00')).toBe(false)
      expect(Time.isValidTimeString('24:01')).toBe(false)
      expect(Time.isValidTimeString('00:60')).toBe(false)
    })
    ...
    it('When sending a valid time string, it should return true', () => {
      ...
      expect(Time.isValidTimeString('24:00')).toBe(true)
      expect(Time.isValidTimeString('00:00')).toBe(true)
      expect(Time.isValidTimeString('12:34')).toBe(true)
    })
  })
  ...
  describe('new Time()', () => {
    ...
    it('When creating a time with a string, it creates the correct time', () => {
      ...
      const shortTime = new Time('23:59')

      expect(shortTime.getHours()).toBe(23)
      expect(shortTime.getMinutes()).toBe(59)
      expect(shortTime.getSeconds()).toBe(0)

      const shortMidnight = new Time('00:00')

      expect(shortMidnight.getHours()).toBe(0)
      expect(shortMidnight.getMinutes()).toBe(0)
      expect(shortMidnight.getSeconds()).toBe(0)
    })
