...
import { StartsAfterOrAt } from '#src/common/index.js'
...
describe('StartsAfterOrAt for Timestamp', () => {
    ...
    it('finds date-time ranges that start after the given timestamp', async () => {
        ...
        const results = await dataSource.manager.find(DateTimeRangeTest, {
            where: {
                id: In([1, 2, 3]),
                range: StartsAfterOrAt(timestamp('2025-02-01T00:00:00Z'))
            }
        })
    ...
    it('includes date-time ranges that start at the given timestamp', async () => {
    ...
    it('excludes date-time ranges that start before the given timestamp', async () => {
        const range1 = new DateTimeRange(timestamp('2025-01-01T00:00:00Z'), timestamp('2025-01-31T23:59:59Z'))
