...
import { StartsAfterOrAt, StartsAfterOrOn } from '#src/common/index.js'
...
describe('StartsAfterOrAt for PlainDate', () => {
    ...
    it('finds date ranges that start after the given date', async () => {
        ...
        const results = await dataSource.manager.find(FiniteDateRangeTest, {
            where: {
                id: In([1, 2, 3]),
                range: StartsAfterOrAt(plainDate('2025-02-01'))
            }
        })
    ...
    it('includes date ranges that start on the given date', async () => {
    ...
    it('excludes date ranges that start before the given date', async () => {
        const range1 = new DateRange(plainDate('2025-01-01'), plainDate('2025-01-31'))
    ...
    it('StartsAfterOrOn works identically to StartsAfterOrAt', async () => {
        const range1 = new DateRange(plainDate('2025-02-01'), plainDate('2025-02-28'))
        const range2 = new DateRange(plainDate('2025-03-01'), plainDate('2025-03-31'))
        ...
        const results = await dataSource.manager.find(FiniteDateRangeTest, {
            where: {
                id: In([8, 9]),
                range: StartsAfterOrOn(plainDate('2025-02-01'))
            }
        })
