...
/** 
 * Checks if a range ends at or before the given range with the `&<` operator. 
 */
export function EndsBeforeOrAt (date: PlainDate): FindOperator<DateRange>
export function EndsBeforeOrAt (ts: Timestamp | Date): FindOperator<DateTimeRange>
export function EndsBeforeOrAt (
...
/**
 * Alias for EndsBeforeOrAt when working with PlainDate values.
 * Checks if a range ends on or before the given date with the `&<` operator.
 */
export function EndsBeforeOrOn (date: PlainDate): FindOperator<DateRange> {
    return EndsBeforeOrAt(date)
}
