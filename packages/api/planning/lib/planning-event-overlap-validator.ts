...
export class PlanningEventOverlapValidator {
   ...
   */
  static overlap (event: PlanningEvent, candidate: PlanningEvent): boolean {
   ...
   */
  static getFirstOverlapDate (event: PlanningEvent, candidate: PlanningEvent): Moment | null {
  ...
  private* getOverlappingDates (): Generator<Moment> {
    for (const date of this.generateOverlappingDates()) {
      if (this.event.exceptions.some(exception => date.isSame(exception.exceptionDate))) {
        continue
      }
      if (this.candidate.exceptions.some(exception => date.isSame(exception.exceptionDate))) {
        continue
      }
  ...
  private* generateOverlappingDates (): Generator<Moment> {
    ...
    const gen = this.generateOverlappingOffsets(
      this.eventPeriod,
      this.candidatePeriod,
      weeksBetween
    )
    ...
    for (const offset of gen) {
      ...
      if (
        (this.eventEnd != null && date.isAfter(this.eventEnd))
        || (this.candidateEnd != null && date.isAfter(this.candidateEnd))
      ) {
        return
      }
   ...
   * @param weekOffset offset between the two events start date (positive if candidate event
   * starts after event, negative if candidate starts before event)
