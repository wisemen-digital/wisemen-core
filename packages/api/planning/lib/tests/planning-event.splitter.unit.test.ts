...
describe('PlanningEventSplitter Unit Test', function () {
  describe('.split', function () {
    it('should return the original event if the events do not overlap', function () {
      const splitTarget = createPlanningEvent({
    ...
    it('should return nothing if non-recurring events overlap completely', function () {
      const splitTarget = createPlanningEvent({
        startDate: moment().format(YYYY_MM_DD),
        endDate: moment().format(YYYY_MM_DD),
    ...
    it('should return nothing if recurring events overlap completely', function () {
      const splitTarget = createRecurringEvent({
        weeksPeriod: 1,
        startDate: moment().format(YYYY_MM_DD),
    ...
    it('should shorten an event if the times overlap', function () {
    ...
    it('should split the event in a head and tail an event if the timeslot envelops another timeslot', function () {
      const today = moment().format(YYYY_MM_DD)
      const splitTarget = createPlanningEvent({
        startDate: today,
        endDate: today,
    ...
    it('should set an end date on the event when the overlap starts in the lifetime of the open event', function () {
      const today = moment().format(YYYY_MM_DD)
      const splitTarget = createRecurringEvent({
        weeksPeriod: 2,
        startDate: today,
    ...
    it('should split an event in a head and tail when open event envelops the new event', function () {
      const today = moment().format(YYYY_MM_DD)
      const splitTarget = createRecurringEvent({
        isRecurring: true,
        weeksPeriod: 2,
    ...
    it('should split the open event into multiple events to account for the overlap period', function () {
      const today = moment().format(YYYY_MM_DD)
      const splitTarget = createRecurringEvent({
        weeksPeriod: 2,
        startDate: today,
    ...
    it('should split the open event into multiple events to account for the overlap period and the time overlap', function () {
      const today = moment().format(YYYY_MM_DD)
      const splitTarget = createRecurringEvent({
        weeksPeriod: 1,
        startDate: today,
