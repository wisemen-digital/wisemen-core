...
import { IsSameOrBeforeDateTimeString } from './is-same-or-before-date-time-string.validator.js'

class TestClass {
  @IsSameOrBeforeDateTimeString((obj: TestClass) => obj.reference)
  dateTimeString: string

  reference: string | null
}

describe('IsSameOrBeforeDateTimeString decorator test', () => {
  ...
  it('should pass validation when the date-time is before the reference by minutes', async () => {
    const testInstance = new TestClass()
  ...
  it('should pass validation when the date-time is before the reference by seconds', async () => {
    const testInstance = new TestClass()
  ...
  it('should fail validation when the date-time is after the reference date-time', async () => {
    ...
    expect(errors[0].constraints?.isSameOrBeforeDateTimeString).toContain('must be a date string before or the same as')
  })
  ...
  it('should pass validation when date-time is milliseconds before reference', async () => {
    const testInstance = new TestClass()
  ...
  it('should pass validation when date-time is before across different days', async () => {
    const testInstance = new TestClass()
  ...
  it('should provide correct error message with property name and reference value', async () => {
    ...
    expect(errors.length).toBe(1)
    expect(errors[0].constraints?.isSameOrBeforeDateTimeString).toBe(
      `dateTimeString must be a date string before or the same as ${testInstance.reference}`
    )
  })
})
